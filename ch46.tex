...
在前序章节中，我们讨论了矩阵乘法（GEMM）的分块（Tiling）策略。由于矩阵规模通常远超单个SM的存
...
本章介绍的 \texttt{float4} 向量化优化旨在同时改善上述两个问题，使屋顶线模型（Roofline Model）中的性能点向更高算力利用率方向移动。
...
时间。屋顶线分析表明，该优化有效缓解了内存带宽压力，同时将性能点推向了计算与内存平衡的过渡区域（Transition Point），验证了向量化加载对双重瓶颈的改善效果。
