...
本章将解释 CUDA 运行时 API（Runtime API）。CUDA 运行时 API 为 CUDA 提供了一个高级接口，使开发者能够高效地利用 NVIDIA GPU 的能力来完成广泛的计算任务。这些 API 简化了管理 GPU 设备、内存分配以及执行并行核函数的过程。

运行时 API 可供程序员用来开发优化或运行 GPU 加速的应用程序。这种方法是一个省时的替代方案，无需阅读 GPU 白皮书中详细的文档。例如，无需深入研究白皮书来查找设备限制和约束。正如在前一章中解释的那样，例如每个块的最大线程数，这些 API 提供了函数，可以直接从 GPU 查询该值。

查看专门用于运行时 API 函数的网页，如所示，有数十个函数，涵盖设备管理、内存处理以及异步计算。如果想从运行时 API 中找到任何函数，可以参考该网页。
...
API函数最常见的例子之一是 \texttt{cudaGetDeviceProperties}函数。该函数旨在查询支持CUDA的GPU（或一般来说NVIDIA GPU）
的属性和能力。这个函数为开发者提供了重要的信息，例如GPU名称、总全局内存、每个块的最大线程数、计算能力以及其他指标。这些丰富的信息和指标使开发者能够优化他们的CUDA应用程序。
现在让我们将这个函数应用到我们的GPU上来查询它的属性。

\subsection{项目创建与代码实现}

开始本章项目时，进入 CUDA 课程文件夹，创建一个新文件。选择 \texttt{section\_4\_01\_runtime\_api}，
将扩展名从 \texttt{.txt} 改为 \texttt{.cu}，因为需要一个 \texttt{.cu} 文件，而非文本文件。然后使用任意编辑器（例如 Visual Studio Code）打开这个文件，点击 \texttt{Ctrl+S} 保存。

打开它，复制代码并粘贴到这个文件中。这段代码的主要目的是查询 GPU 的一些属性，不需要进行任何计算，因此在主函数之前没有 GPU 核函数。
...
\textbf{思考题：} 已知每个 Warp 由 32 个线程组成，根据每块最大线程数 1024，每个块的最大 Warp 数量是多少？
\[\text{Max Warps per Block} = \frac{1024}{32} = 32\]请注意区分"块"（软件概念）和"SM"（硬件概念）。块在 SM 上执行，这个值可用于计算另一个性能指标——占用率（Occupancy）。
...
\begin{enumerate}
    ...
    \item \textbf{效率优势}: 使用运行时 API 节省了大量时间。替代方案是在白皮书中查阅大量内容才能获取每一个参数值。虽然白皮书中有表格包含许多值，但并非全部，且 API 能动态反映当前设备的真实状态。
\end{enumerate}

需要掌握如何使用运行时 API，因为在后续章节中，检查或分析性能时需要这些值。
