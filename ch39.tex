\section{CUDA GPU并行编程：向量归约优化}

\subsection{引言：自动化多级归约}
在上一讲中，我们手动编写了三级 Kernel 启动代码来处理不同规模的部分和。为了提升代码的通用性，我们首先将其重构
为\textbf{While 循环}结构，使其能自动适应任意大小的输入向量。
...
在基础版归约 Kernel 中，我们通过取模运算来筛选活跃线程：
\begin{lstlisting}
// 原始写法：仅允许满足步幅倍数的线程执行
...
\textbf{为什么这不会破坏计算结果？}
\begin{enumerate}
    \item 以 $stride=1$ 为例：线程1 执行 $input[1] += input[2]$，将位置1的值修改为 $v_1+v_2$。
    \item 但在下一步 $stride=2$ 时，只有线程0、2、4... 参与计算。线程0 读取的是 $input[0]$ 和 $input[2]$。
...
\end{itemize}
尽管内存模式不同，但由于后续步骤的访问模式（由 $stride$ 决定）天然跳过了这些被“污染”的位置，最终$input
[0]$ 中的归约结果与基准版本\textbf{完全一致}（浮点精度误差范围内）。

\subsection{总结与最佳实践}

\begin{enumerate}
    \item \textbf{避免在热路径中使用取模/除法}：在 GPU 内核的紧密循环中，应尽可能用位运算（如 \lstinline{&} 代替 \lstinline{\%}）或通过算法重构（如本节所示）来消除这些高延迟指令。
    ...
    \item \textbf{后续方向}：当前版本仍受限于全局内存带宽和非合并访问。下一讲将引入 \textbf{共享内存（Shared Memory）} 进一步优化访存模式。
\end{enumerate}
