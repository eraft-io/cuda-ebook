...
欢迎大家。在今天的文章中，我将探讨英伟达白皮书中概述的 Volta架构及其规格。在早期的文章中，我们已经详细探讨了英伟达
白皮书中常见的关键参数。在深入探讨之前，我想提醒大家观看本节所有先前文章的重要性，因为它们奠定了理解今天主题所必需的基础
知识。熟悉这些参数至关重要，因为我们将在今天的课程中引用并验证它们。

让我们开始一起揭开 Volta 架构的规格。这里我们有 Volta 白皮书，具体来说这是 V100白皮书。浏览文件时，你
可以在开头看到目录。在目录的第二页，你会找到一个名为“关键特性”的部分。点击该标题，我们可以探索其内容。这不仅仅是一系列
改进的列表，更是V100 相比前代重大增强的全面概述。

点击“关键特性”标题后，我们将看到定义 V100 GPU 或 Volta架构的核心特性，从新的流式多处理器（SM）设计到
第二代 NVLink技术等。我们将在今天的文章中讨论所有这些增强功能。回到目录，这份白皮书的一个突出方面是专门聚焦于人工
智能和深度学习。考虑到Volta 架构首次引入了张量核心（Tensor Core）单元，这种对 AI 和深度学习的聚焦是
非常相关的。
...
张量核心是专门设计用来执行矩阵运算的计算单元，这对神经网络的训练和推理阶段至关重要，也是 Volta架构的重大突破。请注
意目录中“深入硬件架构”这个关键标题。这一标题并非 Volta白皮书独有，它在英伟达各款白皮书中反复出现。例如，Pascal 白皮书和最近的 Hopper (H100)白皮书目录中都有相同的术语。这提醒了我们之前讨论过的重要一点：尽管每份白

皮书详述不同的架构，但英伟达白皮书采用了相似的结构方法。此外，Volta白皮书还包含软件增强部分以及提供其他设备详细信息
的附录。

现在，让我们深入浏览白皮书。文中提到 V100 GPU 拥有约 210 亿个晶体管。作为对比，H100 包含 800亿个
晶体管。这意味着从 Volta 到 Hopper，晶体管数量增加了近 4倍。晶体管数量是可用的计算资源指标，更多的晶体管
通常对应硬件单元数量的增加。

\subsection{SM增强}

接下来讨论 Volta 架构的主要关键特性和增强。首先是 \textbf{SM 增强}：
...
编写 CUDA 程序时，还需参考“CUDA计算能力”表，了解最大线程数、块数等硬件限制。如果应用规模超出限制，需将其拆分
为多个内核执行。
...
一是链路数量增加（GPU 间从 1 条增至 2 条以上）；二是链路速度提升。这使得 GPU间通信带宽从 Pascal 的
160\,GB/s 提升至 Volta 的 300\,GB/s。NVLink 是 DGX服务器的标准特性。例如 DGXStation（4 块 V100）和 DGX-1（8 块 V100）。对比 DGX-1的 Pascal 和 Volta 版

本，虽然整机功耗同为 3200\,W，但 Volta版本凭借张量核心在同等能耗下提供了数倍于 Pascal 的计算性能
（能效比提升）。

本节涵盖了 Volta 架构重点，并对比了 Pascal 和 Ampere。建议大家继续阅读白皮书剩余部分。
